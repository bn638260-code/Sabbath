...
| `rhema-stt` | Deepgram STT WebSocket + REST fallback; local Whisper engine (Vosk retained for compatibility) |
...
- **`rhema-stt`** owns network and local STT. Whisper (default local),
  Vosk (compatibility), and Deepgram all emit the same transcript event
  type, so the consumer doesn't know (or care) which engine is wired in.
...
- `rhema-stt` exposes a `whisper` feature (`whisper-rs = "0.16"`,
  `optional = true`) for the local Whisper engine, enabled by default.
  The Vosk and Deepgram paths are always available.
