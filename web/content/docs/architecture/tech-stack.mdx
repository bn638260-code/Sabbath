...
| **STT**       | Local Vosk worker (default), Deepgram WebSocket + REST (`tokio-tungstenite`), or Soniox live streaming |
...
- **Soniox** streams audio over WebSocket with language hints and endpoint
  detection, then emits the same transcript event type.
