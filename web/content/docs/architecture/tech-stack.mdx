...
| **STT** | Local Whisper (`whisper-rs`, default), or Deepgram WebSocket + REST (`tokio-tungstenite`); Vosk retained for compatibility |
...
- **Whisper** runs locally in-process via `whisper-rs`, using the
  lightweight `ggml-tiny.en.bin` model with a Bible-reference prompt. It
  is the default local engine. (Vosk remains available for compatibility
  via a Python worker process.)
