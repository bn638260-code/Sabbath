...
| `download:vosk`              | Download the default local Vosk STT model                                      |
| `download:whisper`           | Download the Whisper tiny English ggml model used by CI/release builds         |
| `build:vosk-sidecar`         | Build the self-contained Vosk worker used by release builds                    |
...
| Reinstall the default local Vosk model | `bun run download:vosk`                                   |
| Reinstall the CI Whisper model         | `bun run download:whisper`                                |
...
\*A successful CI run on a fresh checkout: `setup:all` walks the setup phases, `cargo test --workspace` exercises the Rust crates, then `bun run test` runs the frontend Vitest suite. Idempotent skip lines appear when artifacts already exist, so subsequent runs are much faster. **Click to expand.\***
