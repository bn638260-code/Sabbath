...
| Cutting a release                      | `bun run tauri:build:release`                     |

## CI usage

A successful CI run on a fresh checkout runs setup phases, `cargo test
--workspace`, the frontend Vitest suite, and typechecking. Idempotent skip lines
appear when artifacts already exist, so subsequent runs are much faster.
