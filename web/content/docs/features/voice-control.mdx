...
SabbathCue listens for a small set of voice commands so you don't have to
...
When the detector matches a voice command, such as "read in NIV", the phrase is
highlighted in the transcript and the active-translation badge in the header
...
**Romans 8**"), SabbathCue enters reading mode and the queue locks to that
