...
def parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--rule1-min-trailing-silence", type=float, default=1.2)
    parser.add_argument("--rule2-min-trailing-silence", type=float, default=0.6)
    parser.add_argument("--rule3-min-utterance-length", type=float, default=300.0)
    return parser
...
def one_file(model_dir: Path, pattern: str, label: str) -> str:
    matches = sorted(
        ...
        key=lambda path: (".int8." not in path.name, path.name),
    )
    if not matches:
        raise FileNotFoundError(
            f"Sherpa model is missing {label} file matching {pattern}: {model_dir}"
        )
...
def main() -> int:
    ...
    try:
        ...
        emit(
            {
                "type": "error",
                "message": f"Python package 'sherpa-onnx' is not installed: {exc}",
            }
        )
        return 1
